...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { amount, phoneNumber, payoutType, orderReference } = req.body;

  if (!amount || !phoneNumber || !payoutType) {
  ...
  try {
    ...
    const ref = orderReference || `PAYOUT-MOBILE-${Date.now()}-${Math.floor(Math.random() * 1000000)}`;
    ...
    if (payoutType === 'preview') {
      payoutResp = await axios.post(
        `${CLICKPESA_BASE_URL}/payouts/preview-mobile-money-payout`,
        {
          amount: amount,
          phoneNumber,
          currency: 'TZS',
      ...
      payoutResp = await axios.post(
        `${CLICKPESA_BASE_URL}/payouts/create-mobile-money-payout`,
        {
          amount: amount,
          phoneNumber,
          currency: 'TZS',
    ...
    console.error('ClickPesa Mobile Payout API error:', error?.response?.data || error);
