...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const { futureId, limit = 20, page = 1 } = req.query;
      const skip = (Number(page) - 1) * Number(limit);
      
      // Build filter conditions for orders to find trades where user is buyer or seller
      const userOrders = await prismaRemote.futureOrder.findMany({
        where: { userId },
        select: { id: true }
      });
      ...
      const [trades, count] = await Promise.all([
        prismaRemote.futureTrade.findMany({
          where,
          skip,
          take: Number(limit),
        ...
        prismaRemote.futureTrade.count({ where }),
      ]);
