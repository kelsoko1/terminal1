...
async function createPost(content: string, attachments: any[] = [], userId?: string): Promise<StatusUpdate | null> {
  try {
    const response = await fetch('/api/social/posts', {
      ...
      body: JSON.stringify({ content, attachments, userId })
    });
...
async function fetchStreams(page: number = 1, limit: number = 10, isLive?: boolean): Promise<LiveStream[]> {
  try {
    const response = await fetch(`/api/users/streams?page=${page}&limit=${limit}`);
    const data = await response.json();
    return data;
  } catch (error) {
...
import { AdMobPost } from './AdMobPost'
import { useAuth } from '@/lib/auth/auth-context'
import { KelsokoChallengeList } from '../investor/kelsoko-challenge/KelsokoChallengeList'

// Define Ad type based on AdMobPostProps
interface AdMobAd {
  id: string;
  name: string;
...
export function TikTokStyleFeed() {
  ...
  const [timedAds, setTimedAds] = useState<AdMobAd[]>([]);
  ...
  const [contentType, setContentType] = useState<'feeds' | 'soko' | 'challenges'>('feeds');
  ...
  useEffect(() => {
    const loadChallenges = async () => {
      try {
        ...
        const formattedChallenges = socialChallenges.map((sc: UnifiedChallenge) => ({
          id: sc.id,
          ...
          reward: Number(sc.reward), // Ensure reward is a number
          difficulty: sc.difficulty,
          timestamp: sc.createdAt,
          expiresAt: sc.expiresAt,
          participants: sc.participants,
          status: (sc.status === 'draft' ? 'active' : sc.status) as 'active' | 'completed' | 'expired', // Cast to correct type
          completedBy: sc.submissions?.filter(s => s.status === 'approved').map(s => s.userId) || [],
          proofRequired: typeof sc.proofRequired === 'boolean' ? sc.proofRequired : true,
  ...
  const { ads: admobAds } = useAdMob();
  
  // Create a combined feed that includes both posts and live streams
  const createCombinedFeed = () => {
    const combinedFeed: JSX.Element[] = [];
    let adIndex = 0;
    
    // Add live streams first (they should appear at the top)
    const liveStreamsForFeed = liveStreams.filter(stream => stream.isLive);
    liveStreamsForFeed.forEach((stream, index) => {
      // Insert an ad every 5 items, if available
      if (index > 0 && index % 5 === 0 && admobAds && admobAds.length > 0) {
        const ad = admobAds[adIndex % admobAds.length];
        combinedFeed.push(
          <AdMobPost key={`admob-ad-${ad.id}-${index}`} ad={ad} />
        );
        adIndex++;
      }
      
      // Add live stream as a post
      combinedFeed.push(
        <div key={`stream-${stream.id}`} className="mb-4">
          <Card className="mobile-full-width overflow-hidden border-2 border-primary/20 shadow-sm rounded-xl hover:shadow-md transition-all duration-300 bg-card dark:bg-card/95">
            <div className="flex">
              ...
              <div className="flex flex-col items-center bg-muted/20 dark:bg-muted/10 px-1 sm:px-2 py-3 rounded-l-xl">
                <Button 
                  variant="ghost" 
                  size="sm"
                  className="h-8 w-8 sm:h-7 sm:w-7 p-0 rounded-full hover:bg-muted transition-colors duration-200 touch-manipulation"
                  onClick={(e) => { e.stopPropagation(); }}
                >
                  <ArrowUp className="h-4 w-4" />
                </Button>
                <span className="font-medium text-xs py-1">
                  {stream.viewers}
                </span>
                <Button 
                  variant="ghost" 
                ...
                <div className="flex items-center space-x-2 mb-1 sm:mb-2">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-sm">{stream.userName}</h3>
                      <Badge variant="destructive" className="text-xs px-2 py-0.5 animate-pulse">
                        LIVE
                      </Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">{formatRelativeTime(stream.startTime)}</p>
                  </div>
                </div>
                
                <h4 className="font-medium text-sm mb-2">{stream.title}</h4>
                
                {/* Live Stream Preview */}
                <div className="rounded-xl overflow-hidden mb-4 bg-muted dark:bg-muted/30 relative">
                  {stream.mediaType === 'video' ? (
                    <div className="aspect-video bg-gradient-to-b from-muted to-card relative">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-background/90 to-transparent p-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium">{stream.viewers} watching</span>
                          </div>
                          <Button 
                            size="sm" 
                            className="bg-primary text-primary-foreground hover:bg-primary/90 text-xs"
                            onClick={(e) => { e.stopPropagation(); handlePlayStream(stream); }}
                          >
                            Join Live
                          </Button>
                        </div>
                    ...
                    <div className="aspect-video bg-gradient-to-b from-primary/10 to-background relative p-4 flex flex-col justify-between">
                      ...
                      <div className="mt-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className="h-2 w-2 rounded-full bg-red-500 animate-pulse"></div>
                            <span className="text-xs font-medium">{stream.viewers} listening</span>
                          </div>
                          <Button 
                            size="sm" 
                            className="bg-primary text-primary-foreground hover:bg-primary/90 text-xs"
    ...
    // Add regular posts
    filteredStatusUpdates.forEach((status, index) => {
      const totalIndex = liveStreamsForFeed.length + index;
      
      // Insert an ad every 5 items, if available
      if (totalIndex > 0 && totalIndex % 5 === 0 && admobAds && admobAds.length > 0) {
        const ad = admobAds[adIndex % admobAds.length];
        combinedFeed.push(
          <AdMobPost key={`admob-ad-${ad.id}-${totalIndex}`} ad={ad} />
        );
        adIndex++;
      }
              ...
              <div className="mb-4">
                ...
                <div className="flex justify-end">
                  <Button 
                    ...
                    onClick={() => handleComment(status.id, commentText)}
                  >
                    {isLoadingComments ? (
  ...
  const feedWithAds = createCombinedFeed();
  
  useEffect(() => {
    let adInterval: NodeJS.Timeout | null = null;

    const shouldShowTimedAds = 
    ...
    if (shouldShowTimedAds) {
      const showAd = () => {
        setTimedAds(currentAds => {
          if (admobAds.length > 0) {
            const nextAdIndex = currentAds.length % admobAds.length;
            const newAd = admobAds[nextAdIndex];
            return [...currentAds, { ...newAd, uniqueId: `${newAd.id}-${Date.now()}` }];
          }
          return currentAds;
        });
      };
      ...
      adInterval = setInterval(showAd, 15000);
    } else {
      setTimedAds([]);
    }
  ...
  }, [filteredStatusUpdates, liveStreams, admobAds, contentType, timedAds.length]);
        ...
        <Card className="border-0 shadow-md rounded-lg bg-card dark:bg-card/90 backdrop-blur-sm overflow-hidden h-full">
          <div className="p-6">
                ...
                <DialogContent className="sm:max-w-[500px] rounded-xl border-0 shadow-lg">
                  ...
                  <div className="space-y-4 py-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-4 gap-3 items-center">
                        <Label htmlFor="challenge-reward" className="text-sm">
                          Reward Amount
                        </Label>
                        <div className="col-span-3 relative">
                          ...
                          <Input
                            ...
                            onChange={(e) => setNewChallenge(prev => ({ ...prev, reward: parseInt(e.target.value) || 0 }))}
                          />
                        </div>
                      </div>
                  ...
                  <DialogFooter>
                    ...
                    <Button 
                      ...
                      onClick={async () => {
                        ...
                        if (newChallenge.title && newChallenge.description && newChallenge.reward) {
                          // Create a new challenge using the challenge service
                          try {
                            const socialChallenge = {
                              title: newChallenge.title,
                              description: newChallenge.description || '',
                              reward: Number(newChallenge.reward) || 50,
                            ...
                            const challenge = {
                              ...
                              reward: Number(createdChallenge.reward), // Ensure reward is a number
                              difficulty: createdChallenge.difficulty,
                              timestamp: createdChallenge.createdAt,
                              expiresAt: createdChallenge.expiresAt,
                              participants: 0,
                              status: createdChallenge.status as 'active' | 'completed' | 'expired', // Cast to correct type
                              completedBy: [],
                            ...
                            setChallenges(prev => [{
                              ...challenge,
                              proofRequired: typeof challenge.proofRequired === 'boolean' ? challenge.proofRequired : true
                            }, ...prev]);
                          
                          // Create a post about the challenge
                          setPostContent(`#NERVE I just created a new challenge: "${challenge.title}" with a $${challenge.reward} prize! Check it out in the Challenges tab.`);
                          ...
                          toast({
                            ...
                            description: `Your challenge "${newChallenge.title}" has been created with a $${newChallenge.reward} prize.`,
                          });
                          } catch (error) {
                      ...
                      disabled={isCreatingChallenge || !newChallenge.title || !newChallenge.description || !newChallenge.reward}
                    >
                      {isCreatingChallenge ? (
                        <>
                          <Loader2 className="h-3 w-3 mr-1 animate-spin" />
              ...
              <Dialog open={showAcceptDialog} onOpenChange={setShowAcceptDialog}>
                <DialogContent className="sm:max-w-[500px] rounded-xl border-0 shadow-lg">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      {currentChallenge ? (
                        <span className="text-sm font-medium">"{currentChallenge.title}" - ${currentChallenge.reward} prize</span>
                  ...
                  <DialogFooter>
                    ...
                    <Button 
                      ...
                      onClick={async () => {
                        ...
                        setPostContent(`#NERVE I just accepted the "${currentChallenge.title}" challenge for $${currentChallenge.reward}! Watch me complete it! 🔥`);
            ...
            {contentType === 'challenges' && (
              <KelsokoChallengeList />
            )}

            {/* Empty State for filtered content - only shown for non-soko and non-challenges tabs with no content */}
            {filteredStatusUpdates.length === 0 && timedAds.length === 0 && contentType !== 'soko' && contentType !== 'challenges' && (
            ...
            {/* Timed ads for empty feed */}
            {timedAds.length > 0 && (
              <div className="space-y-4">
                {timedAds.map(ad => (
                  <AdMobPost key={ad.uniqueId} ad={ad} />
                ))}
              </div>
            )}
