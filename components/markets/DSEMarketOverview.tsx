...
import { Instrument, Stock, Bond, REIT, Fund } from '@/lib/types/market'
...
export function DSEMarketOverview({ onSymbolSelect }: DSEMarketOverviewProps) {
  ...
  const renderChangeIndicator = (change: number) => {
    ...
    return (
      <span className={`flex items-center text-sm ${getValueColor(change, marketStatus)}`}>
        {change >= 0 ? (
  ...
  // Type guards for instrument types
  const isStock = (instrument: Instrument): instrument is Stock => {
    return instrument.type === 'stocks';
  };

  const isBond = (instrument: Instrument): instrument is Bond => {
    return instrument.type === 'bonds';
  };

  const isREIT = (instrument: Instrument): instrument is REIT => {
    return instrument.type === 'reits';
  };

  const isFund = (instrument: Instrument): instrument is Fund => {
    return instrument.type === 'funds';
  };
                    ...
                    ?.exchanges[0].instruments.map(stock => {
                      ...
                      return (
                        <tr key={stock.symbol} className="border-b hover:bg-muted/50">
                          ...
                          <td className={`p-2 text-right ${getValueColor(1, getMarketStatus())}`}>{formatNumber(bid)}</td>
                    ...
                    ?.exchanges[0].instruments.map(stock => {
                      ...
                      return (
                        <tr key={stock.symbol} className="border-b hover:bg-muted/50">
                          ...
                          <td className="p-2 text-right">{formatNumber(stock.price)}</td>
                          <td className="p-2 text-right">{renderChangeIndicator(stock.change)}</td>
                          <td className="p-2 text-right">{isStock(stock) ? formatNumber(stock.volume) : '-'}</td>
                    ...
                    ?.regions[0].exchanges[0].instruments.map(bond => {
                      ...
                      return (
                        <tr key={bond.symbol} className="border-b hover:bg-muted/50">
                          ...
                          <td className="p-2 text-right">{isBond(bond) ? bond.yield.toFixed(2) : '-'}</td>
                          <td className="p-2 text-right">{renderChangeIndicator(bond.change)}</td>
                          <td className="p-2">{isBond(bond) ? bond.maturity : '-'}</td>
                          <td className="p-2 text-center">
                            <Button 
                              variant="outline" 
                              size="sm"
                    ...
                    ?.regions[0].exchanges[0].instruments.map(reit => {
                      ...
                      return (
                        <tr key={reit.symbol} className="border-b hover:bg-muted/50">
                          ...
                          <td className="p-2 text-right">{isREIT(reit) ? reit.nav.toFixed(2) : '-'}</td>
                          <td className="p-2 text-right">{isREIT(reit) ? `${reit.yield.toFixed(2)}%` : '-'}</td>
                          <td className="p-2 text-right">{isREIT(reit) ? `${reit.occupancyRate}%` : '-'}</td>
                          <td className="p-2 text-center">
                            <Button 
                              variant="outline" 
                              size="sm"
                    ...
                    ?.regions[0].exchanges[0].instruments.map(fund => {
                      ...
                      return (
                        <tr key={fund.symbol} className="border-b hover:bg-muted/50">
                          ...
                          <td className={`p-2 text-right ${getValueColor(1, getMarketStatus())}`}>{formatNumber(bid)}</td>
                          <td className="p-2 text-right text-red-600">{formatNumber(ask)}</td>
                          <td className="p-2 text-right">{isFund(fund) ? fund.nav.toFixed(2) : '-'}</td>
                          <td className="p-2 text-right">{isFund(fund) ? renderChangeIndicator(fund.ytdReturn) : '-'}</td>
                          <td className="p-2">{isFund(fund) ? fund.category : '-'}</td>
                          <td className="p-2 text-right">{isFund(fund) ? fund.minimumInvestment : '-'}</td>
                          <td className="p-2 text-center">
                            <Button 
                              variant="outline" 
                              size="sm"
