...
const sidebarItems: SidebarItem[] = [
  ...
  { name: 'FX Futures', href: '/broker?tab=fx', icon: Zap },
  { name: 'Commodities', href: '/broker?tab=commodities', icon: Package },
...
export default function BrokerLayout({
  ...
  const isActive = (href: string) => {
    if (href === '/broker' && pathname === '/broker') {
      return true
    }
    if (href !== '/broker' && pathname.startsWith('/broker') && href.includes('?tab=')) {
      const tabName = href.split('?tab=')[1]
      return pathname.includes(tabName) || pathname.includes(`?tab=${tabName}`)
    }
    return false
  }
